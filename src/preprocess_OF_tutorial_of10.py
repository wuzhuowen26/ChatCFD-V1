...
def case_config_collector():
    ...
    # 遍历每个feature目录
    # ['basic', 'compressible', 'heatTransfer', 'lagrangian', 'multiphase', 'DNS', 'combustion', 'incompressible', 'combustion']
                    ...
                    if 'system' in dirs:
                        ...
                        if os.path.isdir(system_dir):
                            ...
                            if required_files.issubset(system_files):
                                ...
                                # 收集指定子目录中的配置文件及其内容
                                config_files = {}
                                for subdir in target_subdirs:       # target_subdirs = {'0', '0.orig', 'system', 'constant'}
...
multiphase_flow_solvers = [
    ...
    "driftFluxFoam","interFoam","interMixingFoam","multiphaseEulerFoam",
    "multiphaseInterFoam","potentialFreeSurfaceFoam","twoLiquidMixingFoam",
    "chtMultiRegionFoam"
]

particle_flow_solvers = [
    "denseParticleFoam","particleFoam"
]

reacting_flow_solvers = [
    "buoyantReactingFoam","chemFoam","coldEngineFoam","reactingFoam","XiFoam",
]
...
def add_case_path_keys(data):
    for case_data in data.values():
        ...
        for file_path in config_files:
            ...
            if len(parts) > 1 and parts[0] == "constant" and parts[-1] == "momentumTransport":
...
def main():
    ...
    all_case_collector = f'{config.Database_OFv24_PATH}/openfoam_cases_of10.json'
    with open(all_case_collector, 'w', encoding='utf-8') as f:
        json.dump(cases_dict_collection, f, indent=4, ensure_ascii=False)

    # 删除配置文件过长的算例
    discrete_tmp_json = f'{config.Database_OFv24_PATH}/discrete_case_config_with_descriptions_of10.json'
    describe_cases(all_case_collector, output_file=discrete_tmp_json)

    output_file = f'{config.Database_OFv24_PATH}/merged_OF_cases_of10.json'
    ...
    ofv24_keywords_file = f'{config.Database_OFv24_PATH}/ofv24_keywords_of10.json'

    with open(ofv24_keywords_file, 'w', encoding='utf-8') as file:
        json.dump(config.global_OF_keywords, file, indent=4)

    processed_merged_OF_cases = f'{config.Database_OFv24_PATH}/processed_merged_OF_cases_of10.json'
...
def read_in_processed_merged_OF_cases():
    ...
    proprocess_case_json_file = f"{config.Database_OFv24_PATH}/processed_merged_OF_cases_of10.json"
